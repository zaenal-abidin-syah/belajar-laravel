    ...
    <form method="post" class="mb-5" action="/dashboard/posts/{{ $post->slug }}" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        ...
        <input type="text" value="{{ old('title', $post->title) }}"
          class="form-control @error('title') is-invalid @enderror" name="title" id="title" required autofocus>
        @error('title')
      ...
      <div class="mb-3">
        ...
        <input type="text" value="{{ old('slug', $post->slug) }}"
          class="form-control @error('slug') is-invalid @enderror" name="slug" id="slug" required>
        @error('slug')
          <div class="invalid-feedback">{{ $message }}</div>
        @enderror
      </div>
